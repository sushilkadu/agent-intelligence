#!/usr/bin/env python3
"""Local, long-running SQS worker adapter for parser-service's Lambda
handler.

See crawler-service's `scripts/run_sqs_worker.py` for the full
rationale -- this is the same pattern, applied to `raw-fetched` ->
`parser.handler.lambda_handler` instead of `crawl-queue` ->
`crawler.handler.lambda_handler`. Purely infrastructure glue: no
business logic lives here, `parser/handler.py` is unmodified, and a
...
from parser.config import RAW_FETCHED_QUEUE_URL, boto3_client_kwargs
from parser.handler import lambda_handler

logger = get_logger("parser-service.worker")
...
def main() -> int:
    if not RAW_FETCHED_QUEUE_URL:
        print("RAW_FETCHED_QUEUE_URL is not set -- nothing to poll.", file=sys.stderr)
        return 1

    sqs_client = boto3.client("sqs", **boto3_client_kwargs())
    logger.info("parser-service SQS worker starting; polling %s", RAW_FETCHED_QUEUE_URL)

    while True:
        try:
            received = sqs_client.receive_message(
                QueueUrl=RAW_FETCHED_QUEUE_URL,
                MaxNumberOfMessages=MAX_MESSAGES,
                WaitTimeSeconds=WAIT_TIME_SECONDS,
            )
        except Exception:
            logger.exception("receive_message failed against %s; retrying shortly", RAW_FETCHED_QUEUE_URL)
            time.sleep(5)
            continue
